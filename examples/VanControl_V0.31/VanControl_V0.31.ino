//
// VanControl_V0.31.ino
//
// VOLKSWAGEN VAN CONTROL FOR JACOBO MONTAÑO FERNÁNDEZ (JackVanControl)
//---------------------******--------------------------//
//
// Include:
//    - DO INCLUDE Communication with Nextion GUI using serial comunication (to be used with JackVan_V9.hmi)
...
int readInterval=5000; //in milliseconds
int controlInterval=500; //in milliseconds
...
int CompThresh=100; // Threshold for compressor pressure
int PLowerThresh=100; // Threshold for deflation pressure
...
//////////////////////////////////////////////////////////////////////
  ...
  if (currentMillis - previousMillisRead >= readInterval){
    ...
    Van1.read_Vbat();
    Van1.read_pressures_Nsamples(20);
    Van1.read_heights_Nsamples_discard(40);
    Van1.read_angle_Nsamples(40);   
...
//    if(0){
      ...
      Van1.read_heights_Nsamples_discard(40);
      Van1.read_pressures_Nsamples(30);
      
      // Controlling Rear Height
      if(abs(desHRear-Van1.HRear)>tolH){ //we have to change Rear Pressure

         flagRear=0; // flag 0 when Height is not correct

         if(desHRear>Van1.PRear){//we have to increase Rear Height
...
//                delay(2000);      
         ...
         else if(desHRear<Van1.HRear){//we have to decrease Rear Height
   //        PRear--; // decrease Rear Height for simulation
           // Activate corresponding delay
              ...
              digitalWrite(rearDownRelayPin, HIGH);
              flagRear=1; // Rear Height is correct!
              #ifdef SerialDEBUG
      ...
      if(abs(desHFront-Van1.HFront)>tolH){ //we have to change Front Pressure
         ...
         flagFront=0; // flag 0 when height is not correct
         if(desHFront>Van1.HFront){//we have to increase Front height
  ...
  //              PFront++; // increase Rear Pressure for simulation
    ...
    //            PFront--; // decrease front Pressure for simulation
              ...
              digitalWrite(frontUpRelayPin, HIGH); // turn it OFF
              digitalWrite(frontDownRelayPin, HIGH); // turn it OFF
              flagFront=1; // Front height is correct!
              #ifdef SerialDEBUG
...
////////////////////////////////////////////////////////////////////////////
  ...
  if (currentMillis - previousMillisSerialPrint >= printSerialInterval){
    ...
    Serial.print("  Desired Height Front (cm): "); Serial.println(desHFront);
    Serial.print("  Desired Height Rear  (cm): "); Serial.println(desHRear);
   }
#endif
